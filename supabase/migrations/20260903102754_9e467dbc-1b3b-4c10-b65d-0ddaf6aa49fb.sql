alter table public.campaigns add column brand_name text not null default '';

-- Backfill from the owning profile
update public.campaigns c
set brand_name = coalesce(p.company_name, p.full_name, 'A brand on Unignored')
from public.profiles p
where p.id = c.brand_id and c.brand_name = '';

-- Keep demo campaigns labelled
update public.campaigns set brand_name = 'Sparkling Drinks Co.' where title like 'Sparkling Yuzu%';
update public.campaigns set brand_name = 'Kampala Eats' where title like 'Kampala Street Food%';
update public.campaigns set brand_name = 'PixelWorks' where title like 'Budget Phone%';

-- Demo seeder should label its campaigns too
...
begin
  ...
  select id into v_s2 from submissions where creator_id = p_user_id and id <> v_s1;
